...
const { OAuthError, Request, Response } = OAuthServer;
...
class OAuth2BaseServer {
  ...
  getClient = async (clientId, clientSecret, done) => {
    try {
      ...
      return done(null, {
        ...
        grants: JSON.parse(result.grants)
      });
    } catch(error) { return done(error); }
  ...
  authenticate = async (request, response) => {
    const _request = this.constructor.formatRequest(request);
    const _response = this.constructor.formatResponse(response);

    await this.server.authenticate(_request, _response)
      .then(result => {
  ...
  requestToken = async (request, response) => {
    ...
    return this.server.token(_request, _response)
  ...
  //
  // Formats request from application/vnd.api+json
  // to application/x-www-form-urlencoded and in the
  // correct format for node-oauth-server
  //
  // @param request { Request }

  static formatRequest(request) {
    const {
      method, query = {}, params: { data = {} }
    } = request;
    ...
    return new Request({
      ...
      query: query,
      method,
      body: Object.keys(data).reduce((obj, key) => {
        obj[snakeCase(key)] = data[key];
        return obj;
      }, {})
    });
  }

  // @static formatRepsonse
  //
  // Formats the Lux response to a node-oauth-server
  // response format
  //
  // @param response { Response }
